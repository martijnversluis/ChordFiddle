  ...
  async.waterfall([
    ...
    function (hamlData, readmeMarkdown, callback) {
      fs.stat("./bundle.js", function (error, jsStats) {
        ...
        callback(error, hamlData, readmeMarkdown, jsStats);
      });
    },

    function (hamlData, readmeMarkdown, jsStats, callback) {
      fs.stat("./bundle.css", function (error, cssStats) {
        if (!error) console.log('CSS bundle mtime: ' + cssStats.mtime);
        callback(error, hamlData, readmeMarkdown, jsStats, cssStats);
      });
    },

    function (hamlData, readmeMarkdown, jsStats, cssStats, callback) {
